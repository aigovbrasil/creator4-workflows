---
title: "Formulário interativo de diagnóstico X-RAY"
date: 2026-04-30
account: leonardo.pdab@gmail.com
uuid: fc3416bc-4079-496b-adc4-79f223f677e3
messages: 2
tags: []
context: ""
---
...
100% mobile-first, funciona em qualquer browser.]
...
Formulário X-RAY rodando inline. O que foi entregue:

```
ESTRUTURA
├── 5 seções · 25 campos
│   ├── S01 · Contexto     → perfil + setor + tempo de mercado
│   ├── S02 · Desafio      → problema + tentativas + urgência (escala)
│   ├── S03 · Alvo         → meta + faturamento atual/meta + prazo
│   ├── S04 · Recursos     → checkboxes múltiplos + investimento + horas
│   └── S05 · Perfil       → experiência + origem + fortes/fracos + palavra do momento
└── S06 · Output           → JSON gerado com session_id XRAY-XXXX

LÓGICA
├── session_id gerado automaticamente no carregamento
├── Estado preservado na navegação back/next
├── questions_answered conta apenas campos preenchidos
└── Botão "Copiar JSON" usa Clipboard API com fallback
...
NEXT_ACTION → hospedar o HTML estático em GitHub Pages para eliminar dependência do Claude.ai do cliente.