---
title: "Estruturando portfólio de skills"
date: 2026-05-16
account: leonardopimentel.knh@gmail.com
uuid: bc38b1bf-ab19-4a3c-a72c-501a7a6396ea
messages: 4
tags: []
context: ""
---
...
Preciso de output estrurado para de sever meu skill portifilo atual
...
Aqui está o mapa completo do seu portfólio. Clique em qualquer skill para ver descrição detalhada e os gatilhos que a ativam.

**Resumo estruturado:**

**3 skills de usuário** (customizadas por você) — o núcleo estratégico do portfólio: `cognitive-framework-router` para pensar estruturado, `research-protocol-diamond` para pesquisa rigorosa, e `visual-canvas-studio` para outputs visuais premium.

**8 skills públicas** — cobrem os formatos de arquivo essenciais (docx, pdf, pptx, xlsx), leitura de uploads, design frontend e auto-conhecimento de produto Anthropic.

**10 skills de exemplo** — biblioteca de referência para casos especializados: comunicação interna, MCP, arte generativa, branding, GIFs, e criação de novas skills.

Use os botões no final do painel para explorar gaps ou subutilizações no portfólio.

---

**Você** _2026-05-16_

create a prompt to replica the ciem widege strcure desieng index in other accounts
...
1	---
    ...
    43	
Vou criar um prompt completo e autocontido que replica a estrutura, design e lógica do painel em qualquer conta Claude.
     1	# Output Contracts
     2	
    ...
    64	
File created successfully: /mnt/user-data/outputs/skill-portfolio-widget-prompt.md
O prompt está pronto. Ele é **autocontido** — não depende de nenhum skill instalado para funcionar.

**Como usar em outra conta:**

1. Abra o arquivo, copie tudo a partir de `## PROMPT`
2. Substitua o bloco `SKILL_DATA` com os skills da conta de destino
3. Cole numa conversa Claude qualquer — o widget será gerado com design idêntico

**O que o prompt especifica:**

- Sistema de cores completo via CSS variables (adapta light/dark mode automaticamente)
- Lógica de toggle dos cards (clique duplo fecha, outro card troca)
- Cálculo dinâmico das métricas do header
- Ícones Tabler via CDN
- Badges coloridos por camada (roxo/verde/cinza)
- Comportamento dos botões `sendPrompt()`
- Lista explícita do que *não* fazer para evitar erros comuns